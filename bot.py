...
def format_strategi(data: Dict[str, Any] = None) -> str:
    ...
    ea_strat = raw.get("strategy") or "-"
    ea_sig = raw.get("signal_type") or "-"
    ea_score = raw.get("signal_score")
    ea_step = raw.get("martingale_step")
    ea_pos = raw.get("positions")
    ea_status = ""
    if ea_strat and ea_strat != "-":
        ea_status = (
            f"\n🤖 <b>Status EA GT (live):</b>\n"
            f"• Strategi aktif : <b>{_h(str(ea_strat))}</b>\n"
            f"• Sinyal terakhir: {_h(str(ea_sig))}"
            + (f" ({ea_score}/10)" if ea_score is not None else "") + "\n"
            f"• Martingale step: {ea_step if ea_step is not None else '-'}\n"
            f"• Posisi terbuka  : {ea_pos if ea_pos is not None else '-'}\n"
        )

    text = (
        "📚 <b>Sistem & Strategi Memburu saldo Gold (XAUUSD)</b>\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "<b>🤖 4 Strategi EA GT Pesaldo (v1.10)</b>\n\n"
        "<b>1. Trobosan (Breakout Martingale)</b>\n"
        "• Entry saat harga menembus High/Low bar GT sebelumnya\n"
        "• SL di sisi lawan bar, TP = range bar\n"
        "• Loss → naik step martingale (instant reverse)\n"
        "• Cocok pasar trending / volatilitas tinggi\n\n"
        "<b>2. Layer / Grid</b>\n"
        "• Tambah posisi searah setiap jarak InpLayerStepPts\n"
        "• Lot naik sesuai multiplier per layer\n"
        "• Rata-rata harga entry, max step terbatas\n"
        "• Cocok ranging dengan bias arah dari sinyal GT\n\n"
        "<b>3. Zigzag (Bouncing Martingale)</b>\n"
        "• Entry bounce dekat High/Low level GT\n"
        "• Buy dekat rendah, Sell dekat tinggi\n"
        "• Martingale jika arah salah\n"
        "• Cocok mean-reversion / julat sempit\n\n"
        "<b>4. Sinyal GT A–E + SL/TP dari GT</b>\n"
        "• Engine sama dengan bot (Momentum, Pullback, Range, Imbalance, Konvergensi)\n"
        "• Entry hanya jika skor ≥ 6\n"
        "• SL/TP dihitung dari level GT (tinggi/rendah/range)\n"
        "• Opsional filter InpRequireSignal di strategi lain\n\n"
        "<b>Sinyal pintar bot (A–E):</b>\n"
        "• A Momentum kuat · B Pullback · C Range · D Imbalance · E Konvergensi\n\n"
        "<b>Risk Management (WAJIB)</b>\n"
        "• Risk per transaksi maksimal 1–2% equity\n"
        "• Max martingale/layer step sesuai InpMaxSteps\n"
        "• Hindari high-impact news tanpa filter\n"
        f"{ea_status}"
        f"{rekomendasi}\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "⚠️ <i>Edukasi & ide sistem. Bukan saran finansial. "
        "Sesuaikan risk management-mu sendiri.</i>"
    )
    return text
